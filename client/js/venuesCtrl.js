(function(){
	...
	angular.module('myApp')
					...
					function handleMouseEvent(event) {
						...
						if(scope.allMarkers.length > 0) {
							for( var i = 0; i < scope.allMarkers.length; i++) {
								...
								if( scope.allMarkers[i]._latlng.lat == attrs.data ) {
					
									scope.allMarkers[i].openPopup();
								}	
							}
		...
		function VenuesCtrl($scope, $timeout, MapService){
			...
			$scope.venueFilter 	= true;
			$scope.sortType    	= "";
			$scope.searchType   = "";
			$scope.searchName 	= "";
			$scope.venueLimit 	= 5;
			$scope.loadMoreVenue = false;
			$scope.toggleVenues = false;

			if( venuesData.length  > 0 ){
				$scope.venues      	= venuesData;
				
				MapService.getSearchVenueMap($scope.venues);
				$scope.allMarkers = MapService.getAllMarkers();
			}
			...
			//(x) amount of times
			$scope.duplicate = function(val, times){
				var str = "",
					i;
			...
			$scope.showMoreVenues = function (){
				if( $scope.venues.length == $scope.venueLimit ) {
					$scope.loadMoreVenue = true;	
				}

				$scope.toggleVenues = true; 
